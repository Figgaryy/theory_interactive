...
$('pg-find').onclick = () => {
  ...
  const inL = FA.runDFA(D, w).accepted;
  ...
  if (!inL) { let sug = null; for (const u of FA.stringsUpTo(D.alphabet, r.n + 3)) { if (u.length >= r.n && FA.runDFA(D, u).accepted) { sug = u; break; } } h += `<div class="ln"><span class="why" style="color:var(--red)">⚠ "${esc(w)}" ∉ L — theorem พูดถึงเฉพาะ ω ∈ L; cycle ที่เจอเป็นแค่การวนใน trap (reject ตลอด)${sug ? ` ลอง ω = <b>${esc(sug)}</b> ที่อยู่ใน L แล้วจะเห็น ✔ ทุก i` : ''}</span></div>`; }
